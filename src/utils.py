...
class Utils:
    ...
    def get_synsets(
        self, word: str
    ) -> Tuple[List[nltk.corpus.reader.wordnet.Synset], int]:
        """
    ...
    def get_lemmas_from_synset(
        self, synset: nltk.corpus.reader.wordnet.Synset
    ) -> List[nltk.corpus.reader.wordnet.Lemma]:
        """
    ...
    def _validate_related_words(self, word1: str, related: List[str]) -> List[str]:
        ...
        for word in related:
            ...
            if (
                self.stemmer.stem(word) != self.stemmer.stem(word1)
                and self._get_fuzz_score(word, word1) <= 80
            ):
                if (
                    "_" not in word
                    and len(word) > 0
                    and word.isalpha() == True
                    and word.lower() != word1.lower()
                ):
