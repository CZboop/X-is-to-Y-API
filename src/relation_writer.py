...
class RelationWriter:
    def __init__(
        self,
        length_limit: int = 12,
        save_path: str = "word_details.csv",
    ...
    def _get_and_save_words_to_db(self) -> List[Dict[str, str]]:
        ...
        for word in range(self.num_words):
            ...
            if random_word not in word_names_list:
                if (
                    len(random_word) <= self.length_limit
                    and "_" not in random_word
                    and random_word.isalpha()
                ):
                    word_synsets, num_synsets = self.utils.get_synsets(random_word)
                    word_lemmas = list(
                        chain.from_iterable(
                            [
                                self.utils.get_lemmas_from_synset(synset)
                    ...
                    validated_synonyms = " ".join(
                        self.utils._validate_related_words(random_word, word_synonyms)
                    )

                    word_antonyms = list(
                        chain.from_iterable(
                            [self.get_antonyms(lemma) for lemma in word_lemmas]
                        )
                    )
                    validated_antonyms = " ".join(
                        self.utils._validate_related_words(random_word, word_antonyms)
                    )

                    word_hyponyms = list(
                        chain.from_iterable(self.get_hyponyms(word_synsets))
                    )
                    validated_hyponyms = " ".join(
                        self.utils._validate_related_words(random_word, word_hyponyms)
                    )

                    word_meronyms = list(
                        chain.from_iterable(self.get_meronyms(word_synsets))
                    )
                    validated_meronyms = " ".join(
                        self.utils._validate_related_words(random_word, word_meronyms)
                    )

                    word_holonyms = list(
                        chain.from_iterable(self.get_holonyms(word_synsets))
                    )
                    validated_holonyms = " ".join(
                        self.utils._validate_related_words(random_word, word_holonyms)
                    )

                    word_entailments = list(
                        chain.from_iterable(self.get_entailments(word_synsets))
                    )
                    validated_entailments = " ".join(
                        self.utils._validate_related_words(
                            random_word, word_entailments
                        )
                    )
                    ...
                    word_details = {
                        "word_name": random_word,
                        "synonyms": cols_list[0],
                        "antonyms": cols_list[1],
                        "hyponyms": cols_list[2],
                        "meronyms": cols_list[3],
                        "holonyms": cols_list[4],
                        "entailments": cols_list[5],
                    }
        ...
        logger.info(
            f"Added {len(word_details_list)} words and their relations to database"
        )
    ...
    def _get_and_save_words_to_csv(self) -> pd.DataFrame:
        ...
        for word in range(self.num_words):
            ...
            if (
                ...
                validated_entailments = " ".join(
                    self.utils._validate_related_words(random_word, word_entailments)
                )
    ...
    def get_synonyms(
        self, synsets: List[nltk.corpus.reader.wordnet.Synset]
    ) -> List[str]:
        """
        ...
        synonyms = list(
            chain.from_iterable(
                chain.from_iterable([wn.synonyms(word) for word in words])
            )
        )
    ...
    def get_hyponyms(
        ...
        hyponym_words = [
            self.utils.get_words_from_synsets(synset) for synset in hyponyms
        ]
    ...
    def get_meronyms(
        ...
        meronym_words = [
            self.utils.get_words_from_synsets(synset) for synset in all_meronyms
        ]
    ...
    def get_holonyms(
        ...
        holonym_words = [
            self.utils.get_words_from_synsets(synset) for synset in all_holonyms
        ]
    ...
    def get_entailments(
        ...
        entailments_words = [
            self.utils.get_words_from_synsets(synset) for synset in entailments
        ]
